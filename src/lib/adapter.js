...
export default (DB) => {
	...
	class MongoAdapter {
		...
		async findByUserCode(userCode) {
			const result = await this.coll().find({ 'payload.userCode': userCode }, this.findOneOptions).limit(1).next();
		...
		async findByUid(uid) {
			const result = await this.coll().find({ 'payload.uid': uid }, this.findOneOptions).limit(1).next();
